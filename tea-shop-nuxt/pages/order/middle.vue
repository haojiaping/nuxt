<template>
   <section>
    <div class="middleWraper">
      <backbar leftColor="#333"
               :fixed='true'
               title="物流信息" @setHeight="setHeight"/>
      <div class="contWraper"
        ...
        <div class="subWraper">
          <div class="subItem df"
               v-for="(item,index) in postageInfo"
               :key="index"
               @click="jumpToEx(item)">
            <div class="itemhd df">
              <span class="f15 fw400"
                    style="color:#D45949;">{{item.state}}</span>
              <span class="f12 fw400"
                    style="color:#999;"
                    v-if="item.postageName!=='-'">{{item.postageName}}:{{item.postageId}}</span>
            </div>
            <div class="subText f14 fw400">{{item.postageContent|formatPost}}</div>
            <div class="van-hairline--top">
              <div class="subpro df"
                   v-for="(elem,index) in item.productList"
                   :key="`good_${index}`">
      ...
      <van-popup v-model="popshow"
        ...
        <div class="popSub df">
          ...
          <div class="popbtn van-hairline--surround"
               @click="popshow = false">
